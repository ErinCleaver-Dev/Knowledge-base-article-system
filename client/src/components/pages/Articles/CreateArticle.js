...
const CreateArticle = ({ classes }) => {
  ...
  const initialEditorState = tempPost
    ? EditorState.createWithContent(convertFromRaw(JSON.parse(tempPost)))
  ...
  const handleEditorChange = (editorState) => {
    const tempPostStorage = JSON.stringify(
  ...
  const handleSubmit = (e) => {
    ...
    //api
    axios
      .post(`${config.URL}api/addArticle`, finalData, {
        headers: {
          secret: "dataSent",
        },
      })
        ...
        <OuterContainer component="form" onSubmit={handleSubmit}>
          ...
          <ContainerGrid container spacing={2}>
            <Grid item xs={12} md={12}>
              ...
              >
                <Editor
                  toolbarHidden={window.innerWidth <= 400 ? true : false}
          ...
          <ContainerGrid container spacing={2} justifyContent={"center"}>
            <Grid item xs={12} md={6}>
              <button className={classes.createArticleButton} type="submit">
                Create An Article
              </button>
            </Grid>
          </ContainerGrid>
