...
const styles = {
  ...
  categorySelect: {
    ...
    backgroundImage:
      "url(https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSZxWreCXesUjtFcPqlnZoOV-ynv6BtB_NirA&usqp=CAU)",
...
const EditArticle = ({ classes }) => {
  ...
  useEffect(() => {
    axios
      .post(`${config.URL}api/getUserSpecificArticle`, { user_id, article_id })
      .then((result) => {
        if (result.data) {
          ...
          setEditorState(
            EditorState.createWithContent(
              convertFromRaw(JSON.parse(result.data.post_content))
            )
          );
      ...
      .catch((e) => {
        setLoadingError(
          "Error - Server down, please bear with us and try again later or refresh!"
        );
      });
  }, []);
  ...
  const handleSubmitChange = (e) => {
    ...
    if (!editorState.getCurrentContent().hasText()) {
      allError = {
        ...allError,
        editorState: "The content should not be empty!",
      };
    } else {
    ...
    //api
    axios
      .post(`${config.URL}api/updateArticle`, finalData, {
        headers: {
          secret: "dataSent",
        },
      })
      ...
      .catch((e) => {
        //console.log(e)
        setSubmitError(
          "Error - Server down, please bear with us, and try later, thanks!"
        );
      });
  };
  ...
  return (
    <React.Fragment>
      ...
      localStorage.getItem("userSecret") === user_id ? (
        <OuterContainer component="form" onSubmit={handleSubmitChange}>
          ...
          <ContainerGrid container alignItems={"center"} spacing={1}>
            <Grid item xs={12} md={12}>
              ...
              <input
                className={classes.titleAndYTInput}
                type="text"
                placeholder="Title *(required)*"
                style={
                  errors.title
                    ? { border: "red solid 3px", marginTop: "10px" }
                    : null
                }
                name="title"
                value={fieldValues.fields.title}
                onChange={handleUserInputState}
              />
            </Grid>
          </ContainerGrid>
          <ContainerGrid container spacing={2}>
            <Grid item xs={12} md={12}>
              <input
                ...
                name="videoLink"
                value={fieldValues.fields.videoLink}
                onChange={handleUserInputState}
              />
            </Grid>
          </ContainerGrid>
          <ContainerGrid container spacing={2}>
            <Grid item xs={12} md={12}>
              ...
              <div
                className={
                  errors.editorState
                    ? `${classes.editorContainer} ${classes.editorContainerError}`
          ...
          <ContainerGrid container spacing={2}>
            ...
            <Grid container alignItems={"center"} item xs={12} md={9}>
              ...
              <select
                ...
                name="category"
                value={fieldValues.fields.category}
                onChange={handleUserInputState}
              >
                <option className={classes.categoryOption} value="">
                  Choose one category
                </option>
                <option className={classes.categoryOption} value="react">
                  React
                </option>
                <option className={classes.categoryOption} value="javascript">
                  Javascript
                </option>
                <option className={classes.categoryOption} value="database">
                  Database
                </option>
                <option className={classes.categoryOption} value="frontend">
                  Frontend
                </option>
                <option className={classes.categoryOption} value="backend">
                  Backend
                </option>
                <option className={classes.categoryOption} value="deployment">
                  Deployment
                </option>
              </select>
            </Grid>
          ...
          <ContainerGrid container spacing={2} justifyContent={"space-between"}>
            ...
            >
              <button
                className={classes.decisionArticleButton}
